...
- [四、MVCC 解决了什么](#四mvcc-解决了什么)
  - [MVCC 为什么不能完全禁止幻读（经典反例）](#mvcc-为什么不能完全禁止幻读经典反例)
...
### MVCC 为什么不能完全禁止幻读（经典反例）

RR 下 MVCC 能防「纯快照读」的幻读，但**一旦事务中对幻影记录做了当前读（UPDATE/DELETE），就会把它拉进自己的可见范围**。

```sql
# T1, RR
BEGIN;
SELECT * FROM hero WHERE number = 30;              -- Empty set，生成 ReadView

# 【T2 执行 INSERT (30, 'g关羽', '魏') 并提交】

UPDATE hero SET country='蜀' WHERE number = 30;    -- ★ 关键：当前读，改写这行
SELECT * FROM hero WHERE number = 30;              -- 这次读到了！❌ 幻读
```

**关键在中间那条 `UPDATE`。** 若去掉它，第二次 `SELECT` 依然是 `Empty set`（MVCC 正常生效）。

#### 可见性算法为什么让它可见

ReadView 的可见性规则第一条：

> **记录的 `DB_TRX_ID` == `creator_trx_id`（这行是我自己改的）→ 可见。**

trx_id 的变化过程：

```
① T2 INSERT 提交后：number=30 这行 DB_TRX_ID = T2
② 若无 UPDATE，T1 快照读判断 DB_TRX_ID(=T2) → T2 是「ReadView 之后才提交」→ 不可见 → 读不到 ✅
③ T1 UPDATE（当前读，读最新已提交值，能找到 T2 插入的行）→ 把 DB_TRX_ID 改写为 T1
④ T1 再快照读，DB_TRX_ID(=T1) == creator_trx_id(T1) → 命中规则第一条 → 可见 → 读到 ❌
```

> **一句话：UPDATE 用「当前读」找到了这条对快照本不可见的记录，并把它的 `DB_TRX_ID` 盖成了 T1 自己的 id，于是它从「别人的、看不见的」变成「我自己的、看得见的」。**

#### 结论

| 场景 | MVCC 能否防幻读 |
|------|----------------|
| 纯快照读之间 | ✅ 能（ReadView 冻结，重复读一致） |
| 快照读中间夹了当前读（UPDATE/DELETE 幻影记录） | ❌ 不能（当前读改写 trx_id，后续快照读可见） |

要彻底防这种幻读，得靠 **next-key 锁**（当前读加锁，从物理上阻止 T2 插入）。这也再次印证：**快照读 vs 当前读混用是一切诡异现象的根源**（同[第三节超卖陷阱](#三快照读--后续写的陷阱rr-下超卖)），而 `DB_TRX_ID` 既是 MVCC 可见性依据、也是隐式锁载体（同[第十二节](#十二隐式锁insert-真的不加锁吗)）。
